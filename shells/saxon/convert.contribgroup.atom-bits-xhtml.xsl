<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="atom:entry/atom:category[@scheme eq 'http://schema.highwire.org/ItemSet/Item#type'][@term eq 'book']">
        ...
        <xsl:variable name="html-output" as="document-node()"
          select="
          saxon:transform(
          saxon:compile-stylesheet(doc('../../xslt/main/book_contribgroup-html.xsl')),
          $source,
          $runtime-params/* )"/>
        <xsl:sequence select="$html-output"/>
      </xsl:when>
      <xsl:when test="atom:entry/atom:category[@scheme eq 'http://schema.highwire.org/ItemSet/Item#type'][@term eq 'chapter']">
        <xsl:variable name="source" as="document-node()" select="doc(resolve-uri(atom:entry/atom:link[@rel eq 'alternate'][@c:role eq 'http://schema.highwire.org/variant/source'][@type eq 'application/xml']/@href,base-uri(.)))"/>
	<xsl:variable name="html-output" as="document-node()"
          select="
          saxon:transform(
          saxon:compile-stylesheet(doc('../../xslt/main/bookpart_contribgroup-html.xsl')),
          $source,
          $runtime-params/* )"/>
        <xsl:sequence select="$html-output"/>
      </xsl:when>
